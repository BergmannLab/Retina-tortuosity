...
for data_idx,data_label in enumerate(["all"]):
	data_path = "/scratch/beegfs/FAC/FBM/DBC/sbergman/retina/DL/output/all_images/retina_%s.pytable"%(data_label,)
	#im_pro = ImageProcess()
	...
	for ii , (img, label, img_orig) in enumerate(dataLoader):	
		...
		feature_value = []
		active_dir = "/scratch/beegfs/FAC/FBM/DBC/sbergman/retina/DL/output/features/21_7_21_all/"
		active_dict = {}
		for f_idx,f in enumerate(model.features):
