% creating of an automated system test (to make sure things work after having refactor)

ARIA_run_tests 0 REVIEW ../../system_test/input/fundus/REVIEW/ ../../AV_maps/ all 0.79 ./ 1 3 11000 20000 100 250 ../../system_test/output

%%%%%%%%% TESTING output
% Read the different data of all the images allowing have all together 
% in read_all_taus
rootdir='/Users/sortinve/develop/retina/preprocessing/helpers/MeasureVessels/src/system_test/output';
rootdir_expected='/Users/sortinve/develop/retina/preprocessing/helpers/MeasureVessels/src/system_test/output_expected';

% Inicializate
read_taus_all = zeros(13,1);
expected_all = zeros(13,1);

% FOR ALL:
% Read the output from output and output expected to check it is the same:
%
% Input:    - *all_stats.tsv from /ouput 
%           - *all_stats.tsv from /ouput_expected
% 
% Output:   - "OK! Expected output and your output is the same"
%           -OR "ERROR! Expected output and your output is NOT the same"


d_all=dir(fullfile(rootdir,'*all_stats.tsv')); 
d_all_expected=dir(fullfile(rootdir_expected,'*all_stats.tsv')); 
% For analize vein or artery you just have to:
%   switch 'all_stats' for 'vein_stats' or 'artery_stats'
...
for i=1:length(d_all_expected)
  aux=tdfread(fullfile(rootdir_expected,d_all_expected(i).name));       
  aux2 = cell2mat(struct2cell(aux));
  expected_all = [expected_all,aux2];  
end

if read_taus_all - expected_all == zeros(13,1)
    disp('OK! Expected output and your output is the same')
else
    disp('ERROR! Expected output and your output is NOT the same')
end
